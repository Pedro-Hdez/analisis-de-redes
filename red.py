...
class Red:
    ...
    def fulkerson(self,fuente,sumidero):
            ...
            while True:
                ...
                if(len_cadena >=1):
                    print(cadena[len_cadena-1].destino.nombre)
    ...
    def flujo_maximo(self,fuentes,sumideros):
        ...
        if len(fuentes) > 1:
            # si hay más de un nodo fuente, creamos un superfuente ficticio
            self.agregar_nodo('A+')
            fuente = self.buscar_nodo('A+')
            for nodo in fuentes:
                self.agregar_arco(fuente.nombre, nodo,0,0,math.inf)
        # revisamos si hay más de un sumidero
        if len(sumideros) > 1:
            # si hay más de un sumidero, creamos un supersumidero ficticio
            self.agregar_nodo('Z-')
            sumidero = self.buscar_nodo('Z-')
            for nodo in sumideros:
                self.agregar_arco(nodo, sumidero.nombre,0,0,math.inf)

        # si solo hay un sumidero o un solo fuente, los buscamos en la red para definirlos
        if(len(fuentes)==1 ):
        ...
        # lista donde guardaremos los nodos ficticios que crearemos en caso de que haya nodos con restricciones
        nodos_ficticios = []
        # lista donde guardaremos los nodos originales de la red que crearemos en caso de que haya nodos con restricciones
        lista_nodos = []
        ...
        if(len(arcos_con_restriccion)> 0):
            ...
            for arco in self.__red[sumideroFicticio]['entrantes']:
                ...
                if(arco.flujo != arco.capacidad):
                    print(arco.res_min, arco.capacidad)
                    # si los arcos del nodo fuente (del arco que tenia restricción) al sumidero ficticion no estan saturados
            ...
            # eliminamos los arcos ficticios que van de sumidero a fuente y viceversa, creados para el caso donde hay arcos con restricciones minimas
        ...
        for nodo in nodos_ficticios:
            ...
            # buscamos los arcos salientes del nodo ficticio creado en la partición del nodo original que tenia restricciones
            for arco in self.__red[nodo_ficticio]["salientes"]:
                lista_arcos_ficticios.append(arco)
            # regresamos los arcos salientes del nodo ficticio al nodo original que tenia restricciones
    ...
    def dfs(self, node,fuente, sumidero, cadena,arcos_visitados):
        bool = False
        # marcamos el nodo vistiado
        node.etiqueta = "marcado"
        # revisamos los arcos salientes del nodo
        for saliente in self.__red[node]["salientes"]: 
            # revisamos si alguno cumple las condiciones para ser parte de la cadena aumentante
            if(saliente.flujo < saliente.capacidad and saliente not in arcos_visitados and saliente.destino.etiqueta != "marcado"): 
                bool = True
                # agregamos el arco a la lista de arcos visitados
                arcos_visitados.append(saliente)
                # agregamos el arco a la cadena aumentante
                cadena.append(saliente) 
                # etiquetamos el arco con el sentido que tiene dentro de la cadena aumentante
                saliente.etiqueta = "sentidoPropio"
                # si el arco nos lleva al sumidero nos detenemos, ya tenemos una cadena aumentante
                if(saliente.destino == sumidero):
                    return cadena
                # si aún no llegamos al sumidero, seguimos buscando
                self.dfs(saliente.destino,fuente,sumidero,cadena,arcos_visitados)
                break

        if(bool == False):
            # si no se encontro algun arco con sentido propio (saliente), revisamos los arcos entrantes del nodo
        ...
        # caso donde llegamos a un camino sin salida o no hay arcos en sentido propio o impropio que cumplan las condiciones para ser aumentantes
        if(bool == False): 
            # revisamos si hay elementos en la cadena    
            if(len(cadena)>=1):
                # si hay elementos en la cadena aumentante, revisamos el sentido que tiene el arco y nos regresamos al nodo anterior
                # si el arco tiene sentido propio nos regresamos al nodo de origen del arco
                if(cadena[len(cadena)-1].etiqueta == "sentidoPropio"):      
                    ...
                    return None
                # si el arco tiene sentido impropio nos regresamos al nodo de destino del arco
        ...
        # si no se encontraron arcos disponibles y no nos podemos regresar, no hay cadena aumentante :(
